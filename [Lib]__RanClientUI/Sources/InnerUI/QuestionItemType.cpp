...
CQuestionItemType::CQuestionItemType () :
    m_bPLAY ( false )
{
}
...
void CQuestionItemType::CreateSubControl ( CString strKeyword )
{
	float fBEGIN_TIME[XCONTROL_SIZE] = { 0.0f, 0.1f, 0.2f };
	float fEND_TIME[XCONTROL_SIZE] = { 0.2f, 0.3f, 0.7f };
	float fRATIO_ARRAY[XCONTROL_SIZE] = { 2.4f, 1.7f, 1.0f };

//	float fBEGIN_TIME[XCONTROL_SIZE] = { 0.0f, 0.2f, 0.3f };
//	float fEND_TIME[XCONTROL_SIZE] = { 0.2f, 0.3f, 0.4f };
//	float fRATIO_ARRAY[XCONTROL_SIZE] = { 1.0f, 2.0f, 3.0f };
...
void	CQuestionItemType::START()
{
	RESET ();
	m_bPLAY = true;
}

void	CQuestionItemType::RESET()
{
	m_fPlayedTime = 0.0f;
...
void CQuestionItemType::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
