...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialogResourceManager::OnCreateDevice( LPDIRECT3DDEVICEQ pd3dDevice )
{
    HRESULT hr = S_OK;
    int i=0;
...
{   
    ...
    if( m_bCaption )
    {
        ...
        WCHAR wszOutput[256];
        StringCchCopyW( wszOutput, 256, m_wszCaption );
        if( m_bMinimized )
            StringCchCatW( wszOutput, 256, L" (Minimized)" );
...
int CDXUTDialogResourceManager::AddFont( LPCWSTR strFaceName, LONG height, LONG weight )
{
    ...
    StringCchCopyW( pNewFontNode->strFace, MAX_PATH, strFaceName );
...
int CDXUTDialogResourceManager::AddTexture( LPCWSTR strFilename )
{
    ...
    StringCchCopyW( pNewTextureNode->strFilename, MAX_PATH, strFilename );
...
HRESULT CDXUTDialogResourceManager::CreateFont( UINT iFont )
{
    ...
    V_RETURN( D3DXCreateFontW ( m_pd3dDevice, pFontNode->nHeight, 0, pFontNode->nWeight, 1, FALSE, DEFAULT_CHARSET, 
...
HRESULT CDXUTDialogResourceManager::CreateTexture( UINT iTexture )
{
    ...
    // Create texture
    D3DXIMAGE_INFO info;
    hr =  D3DXCreateTextureFromFileExW( m_pd3dDevice, strPath, D3DX_DEFAULT, D3DX_DEFAULT, 
...
HRESULT CDXUTStatic::GetTextCopy( LPWSTR strDest, UINT bufferCount )
{
    ...
    StringCchCopyW( strDest, bufferCount, m_strText );
...
HRESULT CDXUTStatic::SetText( LPCWSTR strText )
{
    ...
    StringCchCopyW( m_strText, MAX_PATH, strText); 
    return S_OK;
}
...
HRESULT CDXUTComboBox::AddItem( const WCHAR* strText, void* pData )
{
    ...
    StringCchCopyW( pItem->strText, 256, strText );
    pItem->pData = pData;
...
HRESULT CDXUTListBox::AddItem( const WCHAR *wszText, void *pData )
{
    ...
    StringCchCopyW( pNewItem->strText, 256, wszText );
...
void CDXUTEditBox::SetTextFloatArray( const float *pNumbers, int nCount )
{
    ...
    {
        StringCchPrintfW( wszTmp, 64, L"%.4f ", pNumbers[i] );
        StringCchCatW( wszBuffer, 512, wszTmp );
    }
...
void CUniBuffer::Initialize()
{
    ...
    int len1 = lstrlenW(wszPath);
    int len2 = lstrlenW(UNISCRIBE_DLLNAME);
    ...
    StringCchCatW( wszPath, MAX_PATH, UNISCRIBE_DLLNAME );
...
void CDXUTIMEEditBox::Initialize()
{
    ...
    if( !::GetSystemDirectoryW( wszPath, MAX_PATH+1 ) )
        return;
    StringCchCatW( wszPath, MAX_PATH, IMM32_DLLNAME );
    ...
    if( !::GetSystemDirectoryW( wszPath, MAX_PATH+1 ) )
        return;
    StringCchCatW( wszPath, MAX_PATH, VER_DLLNAME );
