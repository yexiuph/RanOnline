...
void DXUTDisplaySwitchingToREFWarning()
{
    if( DXUTGetShowMsgBoxOnError() )
    {
        ...
        HKEY hKey;
        LONG lResult = RegOpenKeyExW( HKEY_CURRENT_USER, L"Software\\Microsoft\\DirectX 9.0 SDK", 0, KEY_READ, &hKey );
        if( ERROR_SUCCESS == lResult ) 
        {
            DWORD dwType;
            DWORD dwSize = sizeof(DWORD);
            lResult = RegQueryValueExW( hKey, L"Skip Warning On REF", NULL, &dwType, (BYTE*)&dwSkipWarning, &dwSize );
            RegCloseKey( hKey );
        }
        ...
        if( dwSkipWarning == 0 )
        {
            ...
            {
                lResult = RegOpenKeyExW( HKEY_CURRENT_USER, L"Software\\Microsoft\\DirectX 9.0 SDK", 0, KEY_WRITE, &hKey );
                if( ERROR_SUCCESS == lResult ) 
                {
                    dwSkipWarning = 1;
                    RegSetValueExW( hKey, L"Skip Warning On REF", 0, REG_DWORD, (BYTE*)&dwSkipWarning, sizeof(DWORD) );
                    RegCloseKey( hKey );
                }
            }
...
{ 
    ...
    { 
        case WM_INITDIALOG:
            ...
            SetDlgItemTextW( hDlg, 0x101, L"Switching to the Direct3D reference rasterizer, a software device\nthat implements the entire Direct3D feature set, but runs very slowly.\nDo you wish to continue?" ); 
            SetDlgItemTextW( hDlg, IDYES, L"&Yes" );
            SetDlgItemTextW( hDlg, IDNO, L"&No" );
            SetDlgItemTextW( hDlg, IDIGNORE, L"&Don't show again" );
            break;
...
HRESULT DXUTSetMediaSearchPath( LPCWSTR strPath )
{
    ...
    hr = StringCchCopyW( s_strSearchPath, MAX_PATH, strPath );   
    if( SUCCEEDED(hr) )
    {
        // append slash if needed
        size_t ch;
        hr = StringCchLengthW( s_strSearchPath, MAX_PATH, &ch );
        if( SUCCEEDED(hr) && s_strSearchPath[ch-1] != L'\\')
        {
            hr = StringCchCatW( s_strSearchPath, MAX_PATH, L"\\" );
        }
    }
...
HRESULT DXUTFindDXSDKMediaFileCch( WCHAR* strDestPath, int cchDest, LPCWSTR strFilename )
{
    ...
    StringCchPrintfW( strSearchFor, MAX_PATH, L"media\\%s", strFilename ); 
    ...
    StringCchCopyW( strLeafName, MAX_PATH, strFilename ); 
    ...
    StringCchPrintfW( strLeafName, MAX_PATH, L"media\\%s", strFilename ); 
    bFound = DXUTFindMediaSearchParentDirs( strDestPath, cchDest, L".", strLeafName );
    if( bFound )
        return S_OK;
    ...
    StringCchCopyW( strDestPath, cchDest, strFilename );
                                     ...
                                     WCHAR* strExePath, WCHAR* strExeName )
{
    ...
    // Search in .\  
    StringCchCopyW( strSearchPath, cchSearch, strLeaf ); 
    if( GetFileAttributesW( strSearchPath ) != 0xFFFFFFFF )
        return true;

    // Search in ..\  
    StringCchPrintfW( strSearchPath, cchSearch, L"..\\%s", strLeaf ); 
    if( GetFileAttributesW( strSearchPath ) != 0xFFFFFFFF )
        return true;
    ...
    StringCchPrintfW( strSearchPath, cchSearch, L"%s\\%s", strExePath, strLeaf ); 
    if( GetFileAttributesW( strSearchPath ) != 0xFFFFFFFF )
        return true;
    ...
    StringCchPrintfW( strSearchPath, cchSearch, L"%s\\..\\%s\\%s", strExePath, strExeName, strLeaf ); 
    if( GetFileAttributesW( strSearchPath ) != 0xFFFFFFFF )
        return true;
    ...
    {
        StringCchPrintfW( strSearchPath, cchSearch, L"%s%s", s_strSearchPath, strLeaf ); 
        if( GetFileAttributesW( strSearchPath ) != 0xFFFFFFFF )
            return true;
    }
...
bool DXUTFindMediaSearchParentDirs( WCHAR* strSearchPath, int cchSearch, WCHAR* strStartAt, WCHAR* strLeafName )
{
    ...
    {
        StringCchPrintfW( strFullFileName, MAX_PATH, L"%s\\%s", strFullPath, strLeafName ); 
        if( GetFileAttributesW( strFullFileName ) != 0xFFFFFFFF )
        {
            StringCchCopyW( strSearchPath, cchSearch, strFullFileName ); 
            return true;
        }

        StringCchPrintfW( strSearch, MAX_PATH, L"%s\\..", strFullPath ); 
...
HRESULT CDXUTResourceCache::CreateTextureFromFile( LPDIRECT3DDEVICEQ pDevice, LPCWSTR pSrcFile, LPDIRECT3DTEXTUREQ *ppTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateTextureFromFileEx( LPDIRECT3DDEVICEQ pDevice, LPCWSTR pSrcFile, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DTEXTUREQ *ppTexture )
{
    ...
    hr = D3DXCreateTextureFromFileExW( pDevice, pSrcFile, Width, Height, MipLevels, Usage, Format,
    ...
    StringCchCopyW( NewEntry.wszSource, MAX_PATH, pSrcFile );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateTextureFromResource( LPDIRECT3DDEVICEQ pDevice, HMODULE hSrcModule, LPCWSTR pSrcResource, LPDIRECT3DTEXTUREQ *ppTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateTextureFromResourceEx( LPDIRECT3DDEVICEQ pDevice, HMODULE hSrcModule, LPCWSTR pSrcResource, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DTEXTUREQ *ppTexture )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
    ...
    // No matching entry.  Load the resource and create a new entry.
    hr = D3DXCreateTextureFromResourceExW( pDevice, hSrcModule, pSrcResource, Width, Height, MipLevels, Usage,
    ...
    StringCchCopyW( NewEntry.wszSource, MAX_PATH, pSrcResource );
    NewEntry.Width = Width;
    NewEntry.Height = Height;
    NewEntry.MipLevels = MipLevels;
    NewEntry.Usage = Usage;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateCubeTextureFromFile( LPDIRECT3DDEVICEQ pDevice, LPCWSTR pSrcFile, LPDIRECT3DCUBETEXTURE9 *ppCubeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateCubeTextureFromFileEx( LPDIRECT3DDEVICEQ pDevice, LPCWSTR pSrcFile, UINT Size, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DCUBETEXTURE9 *ppCubeTexture )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
    ...
    // No matching entry.  Load the resource and create a new entry.
    hr = D3DXCreateCubeTextureFromFileExW( pDevice, pSrcFile, Size, MipLevels, Usage, Format, Pool, Filter,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateCubeTextureFromResource( LPDIRECT3DDEVICEQ pDevice, HMODULE hSrcModule, LPCWSTR pSrcResource, LPDIRECT3DCUBETEXTURE9 *ppCubeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateCubeTextureFromResourceEx( LPDIRECT3DDEVICEQ pDevice, HMODULE hSrcModule, LPCWSTR pSrcResource, UINT Size, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DCUBETEXTURE9 *ppCubeTexture )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
    ...
    // No matching entry.  Load the resource and create a new entry.
    hr = D3DXCreateCubeTextureFromResourceExW( pDevice, hSrcModule, pSrcResource, Size, MipLevels, Usage, Format,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateVolumeTextureFromFile( LPDIRECT3DDEVICEQ pDevice, LPCWSTR pSrcFile, LPDIRECT3DVOLUMETEXTURE9 *ppVolumeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateVolumeTextureFromFileEx( LPDIRECT3DDEVICEQ pDevice, LPCWSTR pSrcFile, UINT Width, UINT Height, UINT Depth, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DVOLUMETEXTURE9 *ppTexture )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
    ...
    // No matching entry.  Load the resource and create a new entry.
    hr = D3DXCreateVolumeTextureFromFileExW( pDevice, pSrcFile, Width, Height, Depth, MipLevels, Usage, Format,
                                            Pool, Filter, MipFilter, ColorKey, pSrcInfo, pPalette, ppTexture );
    if( FAILED( hr ) )
        return hr;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateVolumeTextureFromResource( LPDIRECT3DDEVICEQ pDevice, HMODULE hSrcModule, LPCWSTR pSrcResource, LPDIRECT3DVOLUMETEXTURE9 *ppVolumeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateVolumeTextureFromResourceEx( LPDIRECT3DDEVICEQ pDevice, HMODULE hSrcModule, LPCWSTR pSrcResource, UINT Width, UINT Height, UINT Depth, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DVOLUMETEXTURE9 *ppVolumeTexture )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
    ...
    // No matching entry.  Load the resource and create a new entry.
    hr = D3DXCreateVolumeTextureFromResourceExW( pDevice, hSrcModule, pSrcResource, Width, Height, Depth, MipLevels, Usage,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateFont( LPDIRECT3DDEVICEQ pDevice, UINT Height, UINT Width, UINT Weight, UINT MipLevels, BOOL Italic, DWORD CharSet, DWORD OutputPrecision, DWORD Quality, DWORD PitchAndFamily, LPCWSTR pFacename, LPD3DXFONT *ppFont )
{
    D3DXFONT_DESCW Desc;
    ...
    StringCchCopyW( Desc.FaceName, LF_FACESIZE, pFacename );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateFontIndirect( LPDIRECT3DDEVICEQ pDevice, CONST D3DXFONT_DESCW *pDesc, LPD3DXFONT *ppFont )
{
    // Search the cache for a matching entry.
    ...
    // No matching entry.  Load the resource and create a new entry.
    hr = D3DXCreateFontIndirectW( pDevice, pDesc, ppFont );
    if( FAILED( hr ) )
        return hr;

    DXUTCache_Font NewEntry;
    (D3DXFONT_DESCW &)NewEntry = *pDesc;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateEffectFromFile( LPDIRECT3DDEVICEQ pDevice, LPCWSTR pSrcFile, const D3DXMACRO *pDefines, LPD3DXINCLUDE pInclude, DWORD Flags, LPD3DXEFFECTPOOL pPool, LPD3DXEFFECT *ppEffect, LPD3DXBUFFER *ppCompilationErrors )
{
    // Search the cache for a matching entry.
    ...
    // No matching entry.  Load the resource and create a new entry.
    hr = D3DXCreateEffectFromFileW( pDevice, pSrcFile, pDefines, pInclude, Flags, pPool, ppEffect, ppCompilationErrors );
    if( FAILED( hr ) )
        return hr;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTResourceCache::CreateEffectFromResource( LPDIRECT3DDEVICEQ pDevice, HMODULE hSrcModule, LPCWSTR pSrcResource, const D3DXMACRO *pDefines, LPD3DXINCLUDE pInclude, DWORD Flags, LPD3DXEFFECTPOOL pPool, LPD3DXEFFECT *ppEffect, LPD3DXBUFFER *ppCompilationErrors )
{
    // Search the cache for a matching entry.
    for( int i = 0; i < m_EffectCache.GetSize(); ++i )
    {
    ...
    // No matching entry.  Load the resource and create a new entry.
    hr = D3DXCreateEffectFromResourceW( pDevice, hSrcModule, pSrcResource, pDefines, pInclude, Flags,
...
HRESULT CDXUTTextHelper::DrawTextLine( const WCHAR* strMsg )
{
    ...
    hr = m_pFont->DrawTextW( m_pSprite, strMsg, -1, &rc, DT_NOCLIP, m_clr );
    if( FAILED(hr) )
...
HRESULT CDXUTTextHelper::DrawTextLine( RECT &rc, DWORD dwFlags, const WCHAR* strMsg )
{
    ...
    HRESULT hr;
    hr = m_pFont->DrawTextW( m_pSprite, strMsg, -1, &rc, dwFlags, m_clr );
    if( FAILED(hr) )
        return DXTRACE_ERR_MSGBOX( "DrawText", hr );
...
HRESULT CDXUTDirectionWidget::StaticOnCreateDevice( IDirect3DDevice9* pd3dDevice )
{
    ...
    V_RETURN( D3DXCreateEffectFromFileW( s_pd3dDevice, str, NULL, NULL, D3DXFX_NOT_CLONEABLE, NULL, &s_pEffect, NULL ) );
    ...
    V_RETURN( D3DXLoadMeshFromXW( str, D3DXMESH_MANAGED, s_pd3dDevice, NULL, 
...
// Desc: Initializes a D3DMATERIALQ structure, setting the diffuse and ambient
//       colors. It does not set emissive or specular colors.
//-----------------------------------------------------------------------------
VOID D3DUtil_InitMaterial( D3DMATERIALQ& mtrl, FLOAT r, FLOAT g, FLOAT b,
                           FLOAT a )
{
...
//-----------------------------------------------------------------------------
VOID D3DUtil_InitLight( D3DLIGHTQ& light, D3DLIGHTTYPE ltType,
