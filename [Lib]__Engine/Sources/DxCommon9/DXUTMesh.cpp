...
{
    StringCchCopyW( m_strName, 512, strName );
    m_pSysMemMesh        = NULL;
    m_pLocalMesh         = NULL;
    m_dwNumMaterials     = 0L;
    m_pMaterials         = NULL;
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::Create( LPDIRECT3DDEVICEQ pd3dDevice, LPCWSTR strFilename )
{
    ...
    // Load the mesh
    if( FAILED( hr = D3DXLoadMeshFromXW( strPath, D3DXMESH_SYSTEMMEM, pd3dDevice, 
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::Create( LPDIRECT3DDEVICEQ pd3dDevice,
...
HRESULT CDXUTMesh::CreateMaterials( LPCWSTR strPath, IDirect3DDevice9 *pd3dDevice, ID3DXBuffer *pAdjacencyBuffer, ID3DXBuffer *pMtrlBuffer )
{
    ...
    if( pMtrlBuffer && m_dwNumMaterials > 0 )
    {
        ...
        m_pMaterials = new D3DMATERIALQ[m_dwNumMaterials];
        ...
        for( DWORD i=0; i<m_dwNumMaterials; i++ )
        {
            ...
            if( d3dxMtrls[i].pTextureFilename )
            {
                ...
                StringCchCatW( strTexture, MAX_PATH, strTextureTemp );

                // Inspect the texture file to determine the texture type.
                if( FAILED( D3DXGetImageInfoFromFileW( strTexture, &ImgInfo ) ) )
                {
                ...
                switch( ImgInfo.ResourceType )
                {
                    ...
                    case D3DRTYPE_CUBETEXTURE:
                    {
                        ...
                        if( SUCCEEDED( D3DXCreateCubeTextureFromFileW( pd3dDevice, strTexture, &pTex ) ) )
                        {
                            // Obtain the base texture interface
                            pTex->QueryInterface( IID_IDirect3DBaseTexture9, (LPVOID*)&m_pTextures[i] );
                            // Release the specialized instance
                    ...
                    case D3DRTYPE_VOLUMETEXTURE:
                    {
                        ...
                        if( SUCCEEDED( D3DXCreateVolumeTextureFromFileW( pd3dDevice, strTexture, &pTex ) ) )
                        {
                            // Obtain the base texture interface
                            pTex->QueryInterface( IID_IDirect3DBaseTexture9, (LPVOID*)&m_pTextures[i] );
                            // Release the specialized instance
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::SetFVF( LPDIRECT3DDEVICEQ pd3dDevice, DWORD dwFVF )
{
...
//       declarations.
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::SetVertexDecl( LPDIRECT3DDEVICEQ pd3dDevice, const D3DVERTEXELEMENT9 *pDecl )
{
    LPD3DXMESH pTempSysMemMesh = NULL;
    LPD3DXMESH pTempLocalMesh  = NULL;
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::RestoreDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMesh::Render( LPDIRECT3DDEVICEQ pd3dDevice, bool bDrawOpaqueSubsets,
...
CDXUTMesh* CDXUTMeshFrame::FindMesh( LPCWSTR strMeshName )
{
    ...
    if( m_pMesh )
        if( !lstrcmpiW( m_pMesh->m_strName, strMeshName ) )
            return m_pMesh;
...
CDXUTMeshFrame* CDXUTMeshFrame::FindFrame( LPCWSTR strFrameName )
{
    ...
    if( !lstrcmpiW( m_strName, strFrameName ) )
        return this;
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFrame::RestoreDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFrame::Render( LPDIRECT3DDEVICEQ pd3dDevice, bool bDrawOpaqueSubsets,
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::LoadFrame( LPDIRECT3DDEVICEQ pd3dDevice,
                             LPD3DXFILEDATA pFileData,
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::LoadMesh( LPDIRECT3DDEVICEQ pd3dDevice,
                            LPD3DXFILEDATA pFileData,
                            CDXUTMeshFrame* pParentFrame )
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::CreateFromResource( LPDIRECT3DDEVICEQ pd3dDevice, LPCWSTR strResource, LPCWSTR strType )
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::Create( LPDIRECT3DDEVICEQ pd3dDevice, LPCWSTR strFilename )
{
    LPD3DXFILE           pDXFile   = NULL;
    LPD3DXFILEENUMOBJECT pEnumObj  = NULL;
    LPD3DXFILEDATA       pFileData = NULL;
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDXUTMeshFile::Render( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX* pmatWorldMatrix )
{
