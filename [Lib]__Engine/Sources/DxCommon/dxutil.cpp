...
const TCHAR* DXUtil_GetDXSDKMediaPath()
{
    ...
    static TCHAR strPath[MAX_PATH] = _T("");
    DWORD dwType;
...
HRESULT DXUtil_FindMediaFile( TCHAR* strPath, TCHAR* strFilename )
{
    ...
    {
        _tcscpy_s( strPath, 1024, strFullPath );
        CloseHandle( file );
        return S_OK;
    }
    ...
    if( INVALID_HANDLE_VALUE != file )
    {
        _tcscpy_s( strPath, 1024, strShortName );
        CloseHandle( file );
        return S_OK;
    }
    
    // last, check if the file exists in the media directory
    _stprintf_s( strPath, 1024, _T("%s%s"), DXUtil_GetDXSDKMediaPath(), strShortName );
    ...
    _tcscpy_s( strPath, 1024, strFilename );
    return E_FAIL;
}
...
{
    TCHAR buffer[256];
    StringCchPrintf( buffer, 256, _T("%s(%ld): "), strFile, dwLine );
    OutputDebugString( buffer );
    OutputDebugString( strMsg );

    if( hr )
    {
        StringCchPrintf( buffer, 256, _T("(hr=%08lx)\n"), hr );
        OutputDebugString( buffer );
    }
