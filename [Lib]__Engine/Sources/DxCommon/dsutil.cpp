...
const	TCHAR*		CSoundFXManager::m_tszFXNames[8] = { "Chorus", "Compressor", "Distortion", "Echo", 
                                   ...
                                   DWORD dwPrimaryBitRate )
{
	...
	if ( m_DlgID != UINT_MAX )
	{
		...
		HWND hSoundDeviceCombo   = GetDlgItem( hWnd, m_DlgID );   
		...
		pSoundGUID = (GUID*) SendMessage( hSoundDeviceCombo, CB_GETITEMDATA, 
												nSoundIndex, 0 );
	}

	// Create IDirectSound using the primary sound device
    if( FAILED( hr = DirectSoundCreate8( pSoundGUID, &m_pDS, NULL ) ) )
...
HRESULT CSoundManager::Get3DListenerInterface( LPDIRECTSOUND3DLISTENER* ppDSListener )
{
    ...
    *ppDSListener = NULL;
    ...
    SecureZeroMemory( &dsbdesc, sizeof(DSBUFFERDESC) );
                ...
                DWORD dwNumBuffers, CWaveFile* pWaveFile )
{
	...
	m_pbUseSound = new BOOL[dwNumBuffers];	
    for( i=0; i<dwNumBuffers; i++ )
	{
        m_apDSBuffer[i] = apDSBuffer[i];
		m_pbUseSound[i] = FALSE;
	}
...
{	
	...
	SAFE_DELETE_ARRAY( m_pbUseSound );
...
LPDIRECTSOUNDBUFFER CSound::GetFreeBuffer( DWORD &BufferID )
{
    ...
    if( i != m_dwNumBuffers )
	{
        return m_apDSBuffer[ BufferID = i ];
	}
    else
	{
		return NULL;
...
//-----------------------------------------------------------------------------
HRESULT CSound::PlayBuffer( DWORD &BufferID, DWORD dwPriority, DWORD dwFlags )
{
    HRESULT hr;
    BOOL    bRestored;
    ...
    LPDIRECTSOUNDBUFFER pDSB = GetFreeBuffer ( BufferID );
...
HRESULT	CSound::StopBuffer ( DWORD BufferID )
{
	HRESULT hr = S_OK;

    if( m_apDSBuffer == NULL )
        return CO_E_NOTINITIALIZED;    
    if( BufferID >= m_dwNumBuffers )
        return E_FAIL;

	if ( m_apDSBuffer )
	{
		hr = m_apDSBuffer[BufferID]->Stop();
		if ( FAILED ( hr ) ) return hr;
	}

	if ( m_pbUseSound )
	{
		m_pbUseSound[BufferID] = FALSE;
	}
...
HRESULT	CSound::ResetBuffer ( DWORD BufferID )
{
    if( m_apDSBuffer == NULL )
        return CO_E_NOTINITIALIZED;
    if( BufferID >= m_dwNumBuffers )
	...
	return m_apDSBuffer[BufferID]->SetCurrentPosition(0);
}
...
BOOL	CSound::IsSoundPlayingBuffer ( DWORD BufferID )
{
    BOOL bIsPlaying = FALSE;
	...
	if( m_apDSBuffer[BufferID] )
    {  
        DWORD dwStatus = 0;
        m_apDSBuffer[BufferID]->GetStatus( &dwStatus );
        bIsPlaying = ( ( dwStatus & DSBSTATUS_PLAYING ) != 0 );    
	}
...
HRESULT	CSoundManager::EnumDevice ( HWND hWnd, int DlgItemID )
{
	HRESULT	hr;
...
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CSoundManager::DSoundEnumCallback( GUID* pGUID, LPSTR strDesc, LPSTR strDrvName,
                                  VOID* pContext )
{
...
CSoundFXManager::CSoundFXManager( )
{
    ...
    SecureZeroMemory( &m_paramsChorus, sizeof( DSFXChorus ) );
    SecureZeroMemory( &m_paramsCompressor, sizeof( DSFXCompressor ) );
    SecureZeroMemory( &m_paramsDistortion, sizeof( DSFXDistortion ) );
    SecureZeroMemory( &m_paramsFlanger, sizeof( DSFXFlanger ) );
    SecureZeroMemory( &m_paramsEcho, sizeof( DSFXEcho ) );
    SecureZeroMemory( &m_paramsGargle, sizeof( DSFXGargle ) );
    SecureZeroMemory( &m_paramsParamEq, sizeof( DSFXParamEq ) );
    SecureZeroMemory( &m_paramsReverb, sizeof( DSFXWavesReverb ) );

    m_dwNumFX = 0;
    SecureZeroMemory( m_rgFxDesc, sizeof( DSEFFECTDESC ) * eNUM_SFX );
    SecureZeroMemory( m_rgRefGuids, sizeof( GUID * ) * eNUM_SFX );
    SecureZeroMemory( m_rgPtrs, sizeof(LPVOID*) * eNUM_SFX );
    SecureZeroMemory( m_rgLoaded, sizeof( BOOL ) * eNUM_SFX );
...
HRESULT CSoundFXManager::EnableGenericFX( GUID guidSFXClass, REFGUID rguidInterface, LPVOID * ppObj )
{
    ...
    SecureZeroMemory( &m_rgFxDesc[m_dwNumFX], sizeof(DSEFFECTDESC) );
...
const	TCHAR*	CSoundFXManager::GetFxName ( DWORD esfxType )
{
	return m_tszFXNames[esfxType];
}