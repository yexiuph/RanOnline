...
void HASH::sha256_begin(sha256_ctx ctx[1])
{
...
void HASH::sha256_compile(sha256_ctx ctx[1])
{   sha2_32t	v[8], j;
...
void HASH::sha256_hash(const unsigned char data[], unsigned long len, sha256_ctx ctx[1])
...
void HASH::sha256_end(unsigned char hval[], sha256_ctx ctx[1])
...
void HASH::sha256(unsigned char hval[], const unsigned char data[], unsigned long len) 
{   sha256_ctx  cx[1];
    
    HASH::sha256_begin(cx);
	HASH::sha256_hash(data, len, cx);
	HASH::sha256_end(hval, cx);
}
...
void HASH::sha512_compile(sha512_ctx ctx[1])
{   sha2_64t    v[8];
    sha2_32t    j;
...
void HASH::sha512_hash(const unsigned char data[], unsigned long len, sha512_ctx ctx[1])
...
static void HASH::sha_end(unsigned char hval[], sha512_ctx ctx[1], const unsigned int hlen)
...
void HASH::sha384_begin(sha384_ctx ctx[1])
{
    ctx->count[0] = ctx->count[1] = 0;
    memcpy(ctx->hash, i384, 8 * sizeof(sha2_64t));
}

void HASH::sha384_end(unsigned char hval[], sha384_ctx ctx[1])
{
    sha_end(hval, ctx, SHA384_DIGEST_SIZE);
}

void HASH::sha384(unsigned char hval[], const unsigned char data[], unsigned long len)
{   sha384_ctx  cx[1];
    
    HASH::sha384_begin(cx);
	HASH::sha384_hash(data, len, cx);
	HASH::sha384_end(hval, cx);
}
...
void HASH::sha512_begin(sha512_ctx ctx[1])
{
    ctx->count[0] = ctx->count[1] = 0;
    memcpy(ctx->hash, i512, 8 * sizeof(sha2_64t));
}

void HASH::sha512_end(unsigned char hval[], sha512_ctx ctx[1])
{
    sha_end(hval, ctx, SHA512_DIGEST_SIZE);
}

void HASH::sha512(unsigned char hval[], const unsigned char data[], unsigned long len) 
{   sha512_ctx  cx[1];
    
    HASH::sha512_begin(cx);
	HASH::sha512_hash(data, len, cx);
	HASH::sha512_end(hval, cx);
}
	...
	unsigned long len, sha2_ctx ctx[1] )
{
    ...
    {
        case 32:
			HASH::sha256_hash(data, len, CTX_256(ctx));
			return;
        case 48:
			HASH::sha384_hash(data, len, CTX_384(ctx)); 
			return;
        case 64:
			HASH::sha512_hash(data, len, CTX_512(ctx)); 
			return;
    }
}
	...
	sha2_ctx ctx[1] )
{
    ...
    {
        case 32: 
			HASH::sha256_end(hval, CTX_256(ctx)); 
			return;
        case 48:
			sha_end(hval, CTX_384(ctx), SHA384_DIGEST_SIZE); 
			return;
        case 64:
			sha_end(hval, CTX_512(ctx), SHA512_DIGEST_SIZE); 
			return;
    }
}
	...
	unsigned long len )
{
	...
	if (HASH::sha2_begin(size, cx) == SHA2_GOOD)
    {
		HASH::sha2_hash(data, len, cx);
		HASH::sha2_end(hval, cx);
		return SHA2_GOOD;
    }
    else
        return SHA2_BAD;
