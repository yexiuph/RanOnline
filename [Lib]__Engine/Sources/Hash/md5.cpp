...
static void HASH::MD5_Transform(
	UINT4 *buf,
	UINT4 *in )
{
	...
	unsigned int inLen )
{
	...
	while (inLen--)
	{
		...
		if (mdi == 0x40)
		{
			...
			HASH::MD5_Transform( mdContext->buf, in );
			mdi = 0;
		}
	}
}
	...
	MD5_CTX *mdContext )
{
	...
	HASH::MD5Update( mdContext, MD5_PADDING, padLen );
	...
	MD5_CTX *mdContext )
{
	...
	HASH::MD5Init( mdContext , seed ) ;

	char buf[2048] ;
	unsigned int trb = 0 ;
	for (;;) {
		int rb = (int)fread ( buf , 1 , 2048 , f ) ;
		if ( size > 0 && rb + trb > size ) rb = size - trb ;
		trb += rb ;
		HASH::MD5Update( mdContext , (unsigned char *) buf , rb ) ;
		if ( rb < 2048 || ( size > 0 && trb >= size ) ) break ;
	}
	fclose ( f ) ;
	HASH::MD5Final( mdContext );
