...
CsheetWithTab::CsheetWithTab(CRect& rect, LOGFONT logfont, CWnd* pParentWnd)
	 : CPropertySheet(IDS_STRING102, pParentWnd),
	 m_bNoTabs(FALSE),
	 m_pWndParent(NULL),
	 m_iDistanceFromTop(0),
	 m_CrowTreePage( logfont ),
	 m_CrowDataPage( logfont ),
	 m_CrowAttackPage( logfont ),
	 ...
	 m_pFont ( NULL )
{
	...
	m_CrowTreePage.SetSheetTab ( this );
	AddPage ( &m_CrowTreePage );

	m_CrowDataPage.SetSheetTab ( this );
	AddPage ( &m_CrowDataPage );

	m_CrowAttackPage.SetSheetTab ( this );
	AddPage ( &m_CrowAttackPage );
...
void CsheetWithTab::ActiveCrowTreePage ()
{		
	m_CrowTreePage.UpdateName ();
	SetActivePage ( CROWTREEPAGE );
}

void CsheetWithTab::ActiveCrowDataPage ( int CurPage, PCROWDATA pCrow )
{
	m_CrowDataPage.SetCallPage ( CurPage );
	if (pCrow)
	{
		m_CrowDataPage.SetCrow ( pCrow );
		SetActivePage ( CROWDATAPAGE );
	}
}

void CsheetWithTab::ActiveCrowAttackPage ( int CurPage, PCROWDATA pCrow )
{
	m_CrowAttackPage.SetCallPage ( CurPage );
	if (pCrow)
	{
		m_CrowAttackPage.SetCrow ( pCrow );
		SetActivePage ( CROWATTACKPAGE );
	}
}

void CsheetWithTab::UpdateCrowTreePage()
{
