...
_MINIDUMP_TYPE g_emMiniDumpTYPE = MiniDumpNormal;

void _SETDUMPTYPE ( int nTYPE )
{
	g_emMiniDumpTYPE = (_MINIDUMP_TYPE) nTYPE;
}
...
static void DumpMiniDump(HANDLE hFile, PEXCEPTION_POINTERS excpInfo)
{
	...
	else
	{
		...
		//	Note : 다음 클라이언트는 메모리를 소량만 덤프함, 아닌 경우 모든 매모리 덤프.
		//
		MiniDumpWriteDump(
			GetCurrentProcess(),
			GetCurrentProcessId(),
...
static void DumpSystemInformation(HANDLE LogFile)
{
	...
	SecureZeroMemory(szModuleName, sizeof(szModuleName));
	if (GetModuleFileName(0, szModuleName, _countof(szModuleName)-2) <= 0)
		StringCchCopy ( szModuleName, MAX_PATH*2, _T("Unknown") );
	...
	if (!GetUserName(szUserName, &UserNameSize))
		StringCchCopy ( szUserName, 200, _T("Unknown"));
								...
								LPCTSTR lpszMessage)
{
	...
	StringCchCopy ( szFileName, MAX_PATH*2, pszFilePart );
	TCHAR *lastperiod = lstrrchr(szFileName, _T('.'));
	if (lastperiod)
		lastperiod[0] = 0;

	// Replace the executable filename with our error log file name
	StringCchCopy ( pszFilePart, MAX_PATH*2, XCRASHREPORT_ERROR_LOG_FILE);
	...
	SecureZeroMemory(szCrashModulePathName, sizeof(szCrashModulePathName));
	...
	StringCchCopy ( pszFilePart, MAX_PATH*2, XCRASHREPORT_MINI_DUMP_FILE);
	...
	else
	{
		...
		StringCchCopy ( pszFilePart, MAX_PATH*2, XCRASHREPORT_CRASH_REPORT_APP );

		TCHAR szCommandLine[MAX_PATH] = {0};
		StringCchCopy ( szCommandLine, MAX_PATH, szModuleName );
