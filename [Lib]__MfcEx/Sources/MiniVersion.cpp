...
{
	SecureZeroMemory(m_szPath, sizeof(m_szPath));
	...
	SecureZeroMemory(m_szCompanyName, sizeof(TCHAR) * (MAX_PATH*2));
	SecureZeroMemory(m_szProductName, sizeof(TCHAR) * (MAX_PATH*2));
	SecureZeroMemory(m_szFileDescription, sizeof(TCHAR) * (MAX_PATH*2));
...
BOOL CMiniVersion::Init()
{
	...
	dwSize = ::GetFileVersionInfoSize((LPTSTR)(LPCTSTR)m_szPath, &dwHandle);
	if (dwSize == 0)
		return FALSE;
		
	m_pData = new BYTE [dwSize + 1];	
	SecureZeroMemory(m_pData, sizeof(BYTE) * (dwSize+1));

	rc = ::GetFileVersionInfo((LPTSTR)(LPCTSTR)m_szPath, dwHandle, dwSize, m_pData);
	if (!rc)
		return FALSE;
...
BOOL CMiniVersion::GetCompanyName(LPTSTR lpszCompanyName, int nSize)
{
	if (!lpszCompanyName)
		return FALSE;
	SecureZeroMemory(lpszCompanyName, nSize);
...
BOOL CMiniVersion::GetFileDescription(LPTSTR lpszFileDescription, int nSize)
{
	if (!lpszFileDescription)
		return FALSE;
	SecureZeroMemory(lpszFileDescription, nSize);
...
BOOL CMiniVersion::GetProductName(LPTSTR lpszProductName, int nSize)
{
	if (!lpszProductName)
		return FALSE;
	SecureZeroMemory(lpszProductName, nSize);
