...
int i;
	for(i=0;i<256;++i)
	{
	    Gamma[i]=(int)(255 * pow(static_cast<float>(i/255.0f),static_cast<float>(m_fGamma)) + (double)0.5);
	}
}
...
{
	int i;
	unsigned int dist=BIG_DISTANCE,d,c;
...
CFile file;
	...
	::SecureZeroMemory(&bFile,sizeof(bFile));
