...
DWORD GetDXVersion()
{
    ...
    {
		StringCchCat( szPath, MAX_PATH, TEXT("\\d3d8.dll"));
        if( SUCCEEDED( GetFileVersion( szPath, strVersion ) ) )
        {
			// Win9x version,  Win2k/WinXP version
            if( strVersion!=TEXT("4.08.0001") || strVersion!=TEXT("5.01.2600") )
            {                    
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT GetFileVersion( TCHAR* szPath, CString &strVersion )
{   
    if( szPath == NULL )
        return E_INVALIDARG;
    ...
    if (cb > 0)
    {
        ...
        if (GetFileVersionInfo( szPath, 0, cb, pFileVersionBuffer))
        {
            ...
            if (VerQueryValue(pFileVersionBuffer, TEXT("\\"), (VOID**)&pVersion, &cb) && 
                pVersion != NULL) 
            {
				strVersion.Format ( "%d.%02d.%04d.%04d", HIWORD(pVersion->dwFileVersionMS), LOWORD(pVersion->dwFileVersionMS),  HIWORD(pVersion->dwFileVersionLS), LOWORD(pVersion->dwFileVersionLS) );
