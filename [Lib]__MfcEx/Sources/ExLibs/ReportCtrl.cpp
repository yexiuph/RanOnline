...
int CReportCtrl::MoveTo(int nItem, int nNewPosition)
{
	...
	const DWORD DATA = static_cast<DWORD>(CListCtrl::GetItemData(nItem));
	CArray<int, int> aImages;
	CStringArray aTexts;
...
BOOL CReportCtrl::SwapItems(int nItem1, int nItem2)
{
	...
	const DWORD DATA1 = static_cast<DWORD>(CListCtrl::GetItemData(nItem1));
	const DWORD DATA2 = static_cast<DWORD>(CListCtrl::GetItemData(nItem2));
...
BOOL CReportCtrl::SetHeaderText(int nColumn, LPCTSTR lpText)
{
	...
	hd.mask = HDI_TEXT;
	hd.pszText = psz;
	hd.cchTextMax = static_cast<int>(_tcslen(psz));
...
void CReportCtrl::_AllocItemMemory(int nItem)
{
	...
	pData->dwData = static_cast<DWORD>(CListCtrl::GetItemData(nItem));
...
BOOL CReportCtrl::SetItem(const LVITEM *pItem)
{
	...
	if ((li.mask & LVIF_PARAM) != 0)
	{
		...
		if (p != NULL)
		{
			p->dwData = static_cast<DWORD>(li.lParam);
			li.lParam = (LPARAM)p;
		}		
	}
...
BOOL CReportCtrl::SetItem(int nItem, int nSubItem, UINT nMask, LPCTSTR lpszItem, int nImage, UINT nState, UINT nStateMask, LPARAM lParam)
{
	if ((nMask & LVIF_PARAM) != 0)
	{
		...
		if (p != NULL)
		{
			p->dwData = static_cast<DWORD>(lParam);
			lParam = (LPARAM)p;
		}		
	}
...
BOOL CReportCtrl::_PartialSort(int nStart, int nEnd)
{
	...
	for (i = 0; i < COUNT; i++)
	{
		...
		aRows[i].dwData = static_cast<DWORD>(CListCtrl::GetItemData(aIndices[i]));
